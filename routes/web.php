...
Route::group(['middleware'=>['Login'],'prefix'=>'/admin/'],function() {
   ...
   Route::get('right', 'Admin\IndexController@right');   //角色授权
    ...
    Route::any('advert','Admin\IndexController@advert');//广告添加
    Route::any('advert_do','Admin\IndexController@advert_do');//广告执行页面
    Route::any('advertlist','Admin\IndexController@advertlist');//广告列表
    Route::any('ad_del/{ad_id}','Admin\IndexController@ad_del');//广告删除
    Route::any('ad_up/{ad_id}','Admin\IndexController@ad_up');//修改
     // Route::any('update/{ad_id}','Admin\IndexController@update');//修改
